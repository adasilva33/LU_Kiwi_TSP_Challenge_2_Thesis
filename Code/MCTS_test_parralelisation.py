...
class MCTS(data_preprocessing):
    ...
    def parallel_select(self, node):
        return self.select(node)

    def parallel_simulate(self, node):
        return self.simulate(node)

    def parallel_expand_node(self, node):
        return self.expand_node(node)

    def parallel_backpropagate(self, node, cost):
        self.backpropagate(node, cost)

    def search_g_1(self):
        with multiprocessing.Pool(
            processes=1
        ) as pool:  # Limit to 1 process for simulation
            iteration = 0
            max_iterations = (
                1000  # Safety mechanism: Break if no progress after 1000 iterations
            )

            while iteration < max_iterations:
                iteration += 1
                self.logger.info(f"Search iteration: {iteration}")

                # Select the node to explore
                node_to_explore = self.select(self.root)

                if node_to_explore is None:
                    self.logger.info("No node selected; breaking out of the loop.")
                    break
                ...
                if not node_to_explore[0]:
                    expanded_node = self.expand_node(
                        node_to_explore[1]
                    )  # Sequential expansion
                    if not expanded_node:
                        self.logger.info("Not expandable so deleted")
                    ...
                    # Simulate the node using a single process
                    simulation = pool.apply(
                        self.parallel_simulate, args=(node_to_explore[1],)
                    )
    ...
    def parallel_simulate_worker(self, node_state, simulation_policy):
        """
        A worker function to run a single simulation.
        """
        current_simulation_state = deepcopy(node_state)
        while current_simulation_state["current_day"] != self.number_of_areas:
            actions = self.possible_flights_from_an_airport_at_a_specific_day_with_previous_areas(
                day=current_simulation_state["current_day"],
                from_airport=current_simulation_state["current_airport"],
            ...
            if action is None:
                ...
                )  # Return a large cost if the simulation fails

            current_simulation_state = self.transition_function(
                current_simulation_state, action
            )

        # Final day handling (return to the starting area)
        if current_simulation_state["current_day"] == self.number_of_areas:
            current_simulation_state["visited_zones"] = current_simulation_state[
                "visited_zones"
            ][1:]
    ...
    def simulate(self, node, k=10):
        """
        Simulate using multiple parallel simulations.
        Args:
            node: The node to start the simulation from.
            k: Number of parallel simulations to run.
        """
        if self.desired_simulation_policy != "random_policy":
            # For non-random policies, just run a single simulation
            return self.parallel_simulate_worker(
                node.state, self.get_simulation_policy()
            )

        with multiprocessing.Pool(processes=k) as pool:
            simulation_policy = self.get_simulation_policy()
            tasks = [(node.state, simulation_policy) for _ in range(k)]
            results = pool.starmap(self.parallel_simulate_worker, tasks)

        # Find the simulation with the smallest total cost
        ...
        with multiprocessing.Pool(processes=4) as pool:
            ...
            while time.time() - start_time < duration:
                while True:
                    ...
                    if (
                        node_to_explore[1].state["current_day"]
                        == self.number_of_areas + 1
                    ):
                        while not node_to_explore[1].parent.is_fully_expanded():
                            node_to_explore = self.select(self.root)
                            self.logger.info(
                                f"Node to explore: {node_to_explore[1].state}"
                            )
                            result = node_to_explore[1].state["total_cost"]
                            self.backpropagate(node_to_explore[1], result)
                    ...
                    if not node_to_explore[0]:
                        ...
                        # Simulate the node using parallel simulations
                        simulation_result = self.simulate(node_to_explore[1], k=5)
                        best_simulation_cost, best_simulation_state = simulation_result

                        if best_simulation_cost < float("inf"):
                            self.logger.info(
                                f"Best result from simulation: {best_simulation_cost}"
                            )
                            key = str(node_to_explore[1].state["current_day"])
                            value_to_add = best_simulation_cost
                            if key in self.simulations_dict:
                                self.simulations_dict[key].append(value_to_add)
                            else:
                                self.simulations_dict[key] = [value_to_add]

                            self.backpropagate(node_to_explore[1], best_simulation_cost)

                        else:
                            self.logger.info("All simulations failed - node deleted")
                            self.delete_node(node_to_explore[1])
                            if len(self.collect_all_nodes()) == 1:
                                self.logger.info(
                                    "Everything has been deleted to the root node"
